...
const MediumClap = ({
  ...
  // ***********************************************
  // CONTROLLED PROPS - FLAG
  // ***********************************************
  const isControlled = !!values && onClap;
  const handleClapClick = () => {
    animationTimeline.replay();
    // ***********************************************
    // CONTROLLED PROPS - STATE CHANGE HANDLER
    // ***********************************************
    isControlled
      ? onClap()
...
const Usage = () => {
  ...
  return (
    <div style={{ width: "100%" }}>
      <MediumClap
        clasName={userCustomStyles.clap}
        onClap={handleClap}
        style={{ border: "2px solid red" }}
      >
        <MediumClap.Icon className={userCustomStyles.icon} />
        <MediumClap.Count className={userCustomStyles.count} />
        <MediumClap.Total style={{ background: "blue" }} />
      </MediumClap>
      {!!count && <div className={null}>{`You have clapped ${count}`}</div>}
    </div>
  );
};
